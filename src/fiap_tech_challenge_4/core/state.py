...
class ModelArtifacts:
    ...
    Holds the loaded model in memory and manages concurrency locks.
    """
    ...
    def __init__(self):
        ...
        # Global lock to prevent concurrent training jobs
        self.training_lock = asyncio.Lock()

    def load_production_model(self, artifact_dir: Path):
        """Loads weights, config, and scalers from disk into memory."""
        if not artifact_dir.exists():
            raise FileNotFoundError(f"Production artifacts not found at {artifact_dir}")
